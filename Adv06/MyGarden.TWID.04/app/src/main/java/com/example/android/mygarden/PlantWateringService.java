...
public class PlantWateringService extends IntentService {
    ...
    protected void onHandleIntent(Intent intent) {
        if (intent != null) {
            ...
            if (ACTION_WATER_PLANT.equals(action)) {    // TODO   дополнительные изменения
    ...
    private void handleActionWaterPlant(long plantId) {  // TODO  дополнительные изменения
        ...
        getContentResolver().update(
                SINGLE_PLANT_URI,               // TODO  дополнительные изменения
                contentValues,
                PlantContract.PlantEntry.COLUMN_LAST_WATERED_TIME + ">?",
                new String[]{String.valueOf(timeNow - PlantUtils.MAX_AGE_WITHOUT_WATER)});

        startActionUpdatePlantWidgets(this);  // TODO  дополнительные изменения
    }
     ...
     */
    private void handleActionUpdatePlantWidgets() {   // TODO  дополнительные изменения
        ...
        if (cursor != null && cursor.getCount() > 0) {
            ...
            cursor.close();
            long waterDelay = timeNow - wateredAt;
            long createDelay = timeNow - createdAt;

            showWater = (waterDelay) > PlantUtils.MIN_AGE_BETWEEN_WATER &&
                    (waterDelay) < PlantUtils.MAX_AGE_WITHOUT_WATER;

            imgRes = PlantUtils.getPlantImageRes(this, createDelay, waterDelay, plantType);
        ...
        PlantWidgetProvider.updatePlantWidgets(this, appWidgetManager,
                imgRes, plantId, showWater, appWidgetIds);
    }
}
